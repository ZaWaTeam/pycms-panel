<template>
    <div class="widget-settings">
        ...
        <VWindow v-model="currentTab" style="width: 100%;" class="mt-5">
            <VWindowItem value="Content" class="widget-settings-card">
                <div v-for="form, index in getForms" :key="form.name" class="widget-setting-option">
                    <VTextField :disabled="!form.body.available" class="form-control" :label="form.body.title"
                        :placeholder="form.body.hint" v-if="isInput(form.body.type) && form.body.category === 'content'"
                        :type="inputAttr(form.body.type)" v-model="(modelForm as any).content[form.name].value" />
                    <VTextarea :disabled="!form.body.available" :label="form.body.title" :placeholder="form.body.hint"
                        v-else-if="isTextarea(form.body.type) && form.body.category === 'content'"
                        v-model="(modelForm as any).content[form.name].value"></VTextarea>
                    <QuillEditor :disabled="!form.body.available" theme="snow" class="qeditor" :options="QuillSettings"
                        v-else-if="isWswyig(form.body.type) && form.body.category === 'content'" content-type="html"
                        v-model:content="(modelForm as any).content[form.name].value">
                    </QuillEditor>
                    <div class="code-content-editor"
                        v-else-if="isCodeEditor(form.body.type) && form.body.category === 'content'">
                        <VAceEditor :disabled="!form.body.available" :options="editorSettings"
                            v-model:value="(modelForm as any).content[form.name].value" theme="monokai"
                    ...
                    <div v-else-if="isSelect(form.body.type) && form.body.category === 'content'">
                        <VSelect v-model:model-value="(modelForm as any).content[form.name].value" :multiple="selectMultiple(form.body.type)" :items="form.body.options" :label="form.body.title" variant="filled" />
                    </div>
                    <hr class="mt-5 mb-5" v-if="form.body.category === 'content'">
                </div>
            </VWindowItem>
            <VWindowItem value="Appearance">
                <div v-for="form, index in getForms" :key="form.name" class="widget-setting-option">
                    <VTextField :disabled="!form.body.available" class="form-control" :label="form.body.title"
                        ...
                        :type="inputAttr(form.body.type)" v-model="(modelForm as any).props[form.name].value" />
                    <VTextarea :disabled="!form.body.available" :label="form.body.title" :placeholder="form.body.hint"
                        v-else-if="isTextarea(form.body.type) && form.body.category === 'appearance'"
                        v-model="(modelForm as any).props[form.name].value"></VTextarea>
                    <QuillEditor :disabled="!form.body.available" theme="snow" class="qeditor" :options="QuillSettings"
                        v-else-if="isWswyig(form.body.type) && form.body.category === 'appearance'" content-type="html"
                        v-model:content="(modelForm as any).props[form.name].value">
                    </QuillEditor>
                    <div class="code-editor"
                        v-else-if="isCodeEditor(form.body.type) && form.body.category === 'appearance'">
                        <VAceEditor :disabled="!form.body.available" :options="editorSettings"
                            v-model:value="(modelForm as any).props[form.name].value" theme="monokai"
                            :lang="codeAttr(form.body.type)" style="height: 300px"></VAceEditor>
                        <div class="display-ordered">
                            <small class="text-muted" v-if="isArray(form.body.hint)" v-for="hint in form.body.hint">{{ hint
                            }}</small>
                    ...
                    <div class="color-picker" v-else-if="isColor(form.body.type) && form.body.category === 'appearance'">
                        ...
                        <!-- <ColorPicker picker-type="chrome" :is-widget="true"
                            v-model="(modelForm as any).props[form.name].value"></ColorPicker> -->
                        <input type="color" :disabled="!form.body.available"
                            v-model="(modelForm as any).props[form.name].value">
                    </div>
                    <div v-else-if="isDimension(form.body.type) && form.body.category === 'appearance'">
                        ...
                        <Dimensions :disabled="!form.body.available" :data="modelForm" :form="form"
                            v-model:dimension-unit="(modelForm as any).props[form.name].unit"
                            v-model:value="(modelForm as any).props[form.name].value"></Dimensions>
                    </div>
                    <div v-else-if="isSelect(form.body.type) && form.body.category === 'appearance'">
                        <VSelect v-model:model-value="(modelForm as any).props[form.name].value" :multiple="selectMultiple(form.body.type)" :items="form.body.options" :label="form.body.title" variant="filled" />
                    </div>
...
/* Tab Settings */
const tabOptions = ["Content", "Appearance", "Settings"];
...
const getForms = computed(() => {
    const returnValue = props.modelForm.widget.settingTypes;
    let returnList = [];
